...
def update_data(move, state):
    ...
    if "game_over" in data:
        ...
        for i,move in enumerate(moves):
            if i != 0: readmeMoves += ", "
            readmeMoves += f"<a href='https://github.com/Coding4Hours/Coding4Hours/issues/new?title=Othello:+{chr(65+move[1])}{move[0]}&body=Please+do+not+change+the+title.+Just+click+\"Submit+new+issue\".+You+do+not+need+to+do+anything+else.+%3AD'>{chr(65+move[1])}{move[0]}</a>"
        readmeMoves += "</p>\n"
        readmeMoves += "</details>\n"

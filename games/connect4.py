...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range(8):
            link = f'COL {i}'
            if data["board"][i][0] == -1:
                link = f'[COL {i}](https://github.com/Coding4Hours/Coding4Hours/issues/new?title=Connect4:+{i}&body=Please+do+not+change+the+title.+Just+click+"Submit+new+issue".+You+do+not+need+to+do+anything+else.+%3AD)'
            value += f'| {link} '
        value += '|\n' + '| :-: ' * 8 + '|\n'
